...
function ReviewModal({ isOpen, onClose, course, user, onReviewSuccess }) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
            <div className="bg-gray-900 border border-gray-700 rounded-2xl w-full max-w-md shadow-2xl p-6 animate-in">
                <div className="flex justify-between items-center mb-5">
                    <h3 className="text-lg font-extrabold text-white">⭐ Rate This Course</h3>
                    <button onClick={onClose} className="text-gray-400 hover:text-white text-xl font-bold">✕</button>
                </div>

                <p className="text-sm text-gray-400 mb-4">You're reviewing: <span className="text-white font-semibold">{course.title}</span></p>

                <form onSubmit={handleSubmit} className="space-y-4">
                    <div className="flex flex-col items-center gap-2 py-3">
                        <p className="text-xs text-gray-400 uppercase font-bold tracking-wide">Your Rating</p>
                        <StarRating rating={rating} interactive onRate={setRating} size="lg" />
                        <p className="text-sm text-amber-400 font-bold h-5">
                            {rating > 0 ? ['', 'Poor', 'Fair', 'Good', 'Very Good', 'Excellent'][rating] : ''}
                        </p>
                    </div>

                    <div>
                        <label className="block text-xs font-bold uppercase text-gray-500 mb-1">Your Review (Optional)</label>
                        <textarea
                            value={comment}
                            onChange={e => setComment(e.target.value)}
                            placeholder="Share what you liked or disliked about this course..."
                            rows={4}
                            className="w-full px-4 py-3 rounded-xl bg-gray-800 border border-gray-700 text-sm text-white outline-none focus:border-blue-500 resize-none"
                        />
                    </div>
                    ...
                    <button
                        ...
                        className="w-full bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-400 hover:to-orange-400 text-black font-extrabold py-3 rounded-xl transition-all hover:scale-[1.01] disabled:opacity-50 disabled:cursor-not-allowed shadow-lg"
                    >
    ...
    return (
        <div className="fixed inset-0 z-40 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
            <div className="bg-gray-900 border border-gray-700 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl">
                <div className="sticky top-0 bg-gray-900 border-b border-gray-800 flex justify-between items-center px-6 py-4 z-10">
                    <h3 className="text-lg font-extrabold text-white">Course Details</h3>
                    <button onClick={onClose} className="text-gray-400 hover:text-white text-xl font-bold">✕</button>
                </div>
                    ...
                    <div className="p-6 space-y-6">
                        ...
                        <div>
                            <div className="flex items-center gap-2 mb-2">
                                <span className="text-xs font-bold bg-blue-500/20 text-blue-400 px-2 py-1 rounded-lg">{course.category}</span>
                                <span className="text-xs font-bold bg-purple-500/20 text-purple-400 px-2 py-1 rounded-lg">{course.level}</span>
                            </div>
                            <h2 className="text-2xl font-extrabold text-white mb-1">{course.title}</h2>
                            <p className="text-gray-400 text-sm">{course.description}</p>
                            <p className="text-sm text-gray-500 mt-2">by <span className="text-cyan-400 font-semibold">{course.instructorName}</span></p>
                        </div>

                        {/* Stats */}
                        <div className="grid grid-cols-3 gap-4">
                            <div className="bg-gray-800 rounded-xl p-3 text-center">
                                <div className="text-amber-400 font-extrabold text-lg">{course.averageRating.toFixed(1)}</div>
                            ...
                            <div className="bg-gray-800 rounded-xl p-3 text-center">
                                <div className="text-green-400 font-extrabold text-lg">{course.totalEnrollments}</div>
                                <div className="text-xs text-gray-500">students</div>
                            </div>
                            <div className="bg-gray-800 rounded-xl p-3 text-center">
                                <div className="text-cyan-400 font-extrabold text-lg">{course.price} 🪙</div>
                        ...
                        <div className="space-y-3">
                            {isEnrolled ? (
                                <div className="flex items-center gap-3">
                                    <div className="flex-1 bg-green-500/10 border border-green-500/30 text-green-400 font-bold text-sm py-3 px-4 rounded-xl text-center">
                                        ✓ Enrolled
                                    </div>
                                    {!hasReviewed && (
                                        <button
                                            onClick={() => setShowReviewModal(true)}
                                            className="flex-1 bg-amber-500 hover:bg-amber-400 text-black font-extrabold text-sm py-3 rounded-xl transition-all hover:scale-[1.01]"
                                        >
                                            ⭐ Leave a Review
                                        </button>
                                    )}
                                    {hasReviewed && (
                                        <div className="flex-1 bg-gray-800 text-gray-400 text-sm py-3 rounded-xl text-center font-semibold">
                                            ✓ Reviewed
                                        </div>
                                    )}
                                </div>
                                ...
                                <button
                                    ...
                                    disabled={enrolling}
                                    className="w-full bg-gradient-to-r from-blue-600 to-cyan-500 hover:from-blue-500 hover:to-cyan-400 text-white font-extrabold py-3 rounded-xl transition-all hover:scale-[1.01] shadow-lg disabled:opacity-50"
                                >
                                    {enrolling ? '⏳ Processing...' : `🎓 Enroll for ${course.price} Tokens`}
                                </button>
                            )}
                            {enrollError && <p className="text-red-400 text-sm font-medium text-center">{enrollError}</p>}
                        </div>
                                    ...
                                    {reviews.map(review => (
                                        <div key={review._id} className="bg-gray-800 rounded-xl p-4">
                                            <div className="flex justify-between items-start mb-1">
                                                <span className="text-sm font-bold text-white">{review.studentName}</span>
...
function CourseCard({ course, isEnrolled, onClick }) {
    ...
    return (
        <div
            onClick={onClick}
            className="bg-[#111827] border border-gray-800 hover:border-blue-500/50 rounded-2xl overflow-hidden shadow-md hover:shadow-xl hover:shadow-blue-900/20 transition-all duration-300 cursor-pointer group"
        >
            {/* Thumbnail */}
            <div className="h-44 bg-gradient-to-br from-blue-900/40 via-purple-900/30 to-gray-900 flex items-center justify-center relative overflow-hidden">
                {course.thumbnail ? (
                ...
                {isEnrolled && (
                    <div className="absolute top-3 right-3 bg-green-500 text-white text-[10px] font-extrabold px-2.5 py-1 rounded-full">
                        ✓ Enrolled
                    </div>
                )}
                <div className="absolute bottom-3 left-3">
                    <span className={`text-xs font-bold px-2.5 py-1 rounded-full ${categoryColors[course.category] || categoryColors.Other} backdrop-blur-sm`}>
                        {course.category}
                    </span>
                </div>
            </div>
            ...
            {/* Content */}
            <div className="p-5">
                <h3 className="text-base font-extrabold text-white group-hover:text-blue-400 transition-colors line-clamp-2 mb-1">
                    {course.title}
                </h3>
...
export default function Courses() {
    ...
    return (
        <div className="min-h-screen bg-[#0B0F19] text-gray-100 font-sans">
            {/* Page Hero */}
            <div className="relative overflow-hidden bg-gradient-to-br from-[#0B0F19] via-[#111827] to-[#0B0F19] border-b border-gray-800">
                ...
                <div className="relative max-w-7xl mx-auto px-4 lg:px-8 py-16">
                    <h1 className="text-4xl lg:text-5xl font-extrabold text-white mb-3 tracking-tight">
                        Explore <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-cyan-300">Courses</span>
                    </h1>
                    <p className="text-gray-400 text-lg max-w-xl">
                        Learn from expert instructors. Enroll with tokens, study at your own pace, and share your experience.
                    </p>
                </div>
            </div>
            ...
            <div className="max-w-7xl mx-auto px-4 lg:px-8 py-8">
                ...
                <div className="flex flex-col sm:flex-row gap-4 mb-8">
                    ...
                    <select
                        ...
                        className="px-4 py-2 rounded-xl bg-gray-800 border border-gray-700 text-sm font-semibold text-gray-300 outline-none cursor-pointer focus:border-blue-500 min-w-[180px]"
                    >
                ...
                <p className="text-sm text-gray-500 mb-6">
                    Showing <span className="text-blue-400 font-bold">{courses.length}</span> courses
